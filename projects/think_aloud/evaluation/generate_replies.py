...
from repliers import RLReplier, NSPReplier, LenkaReplier
...
if __name__ == "__main__":
    ...
    parser.add_argument('--br_dir', default='brain_responses', type=str,
                        help="Folder containing brain_responses to respond to.")
    parser.add_argument('--mode', default='NSP', type=str, choices=['RL', 'NSP', 'Lenka'],
                        help="Thought selection method: {'RL', 'NSP', 'Lenka'}")
    parser.add_argument('--savefile', default='../next_sentence_prediction/model', type=str,
                        help='Path to BERT for NSP (--method=NSP) or RL JSON (--method=RL)')
    args = parser.parse_args()
    print('\nEvaluating brain responses from {} with mode={} and {}'.format(args.br_dir, args.mode, args.savefile))
