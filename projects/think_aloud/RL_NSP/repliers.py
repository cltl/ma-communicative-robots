...
class Replier(BasicReplier):
    ...
    def _phrase_overlaps(all_overlaps, utterance):
        ...
        if (
            len(overlaps) == 2
            and overlaps[0]["_entity"]["_label"] == overlaps[1]["_entity"]["_label"]
        ):
            overlaps = overlaps[:1]
        ...
        if len(overlaps) < 2:
            ...
            if entity_role == "subject":
                ...
                types = (
                    " or ".join(overlaps[0]["_entity"]["_types"])
                    if overlaps[0]["_entity"]["_types"]
                    else "things"
                )
...
class RLReplier(Replier):
    ...
    def reply_to_statement(self, brain_response):
        ...
        thoughts = thoughts_from_brain(
            brain_response
        )  # {thought_descr:(type, thought_info)}
        ...
        thought_info = casefold_capsule({"thought": thought_info}, format="natural")[
            "thought"
        ]
