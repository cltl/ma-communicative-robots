...
class EMISSOR:
    def __init__(self, speaker):
        ...
        self.__scenario = self.__storage.create_scenario(self.__scenario_id, time_id, None, self.__agent)
    ...
    def create_scenario(scenario_path, scenario_id):
        ...
        text_sig_path = os.path.abspath(os.path.join(scenario_path, Modality.TEXT.name.lower()))
    ...
    def add_text_signal(self, text):
        ...
        time_id = int(time.time() * 1e3)
        signal = TextSignal.for_scenario(self.__scenario.id, time_id, time_id, [], text, [])
