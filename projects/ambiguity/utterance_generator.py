...
class Template:
  ...
  def __init__(self, elements, prefixes=['Here', 'And here'], infixes=['', 'By the way, ', 'Oh, have I mentioned, '], postfixes=['.']):
...
RELATION_TYPES = ['colleague', 'spouse', 'sibling', 'unmarried_partner', 'mother', 'father' 'child'
...
def process_person_dict(pers):
  ...
  for relation in RELATION_TYPES:
    if pers.get(relation):
      for fname in pers.get(relation):  #the other persons with whom the person in the photo has a relation (from RELATION_TYPES) with.
        person.relations.append(Relation(person, relation, Person(fname))) #this appends: e.g. Emily Waltham --spouse--> Ross Geller
  #a = Person()
  #a.relation_to =  Rel
  return person
...
def get_picture_dicts(directory):
  ...
  for picture_name in os.listdir(directory):
    for json_filename in os.listdir(os.path.join(directory, picture_name)):
      results[picture_name] = json.load(open(os.path.join(directory,
                                                          picture_name,
                                                          json_filename)))  
  return results
...
POSSESSIVE_RELATIONS = ['spouse', 'unmarried_partner', 'sibling', 'mother', 'father', 'child', 'colleague', 'friend']
...
class Generator:
  ...
  def generate_random_utterance(self, template, persons=[], ambiguity_level=None, person=None, facts_to_mention=1):
    ...
    if len(persons) > 1: #if a photo has multiple people, introduce them appropriately
    ...
    if person in self.mentioned_people: #if so, check what facts have already been mentioned
    ...
    chosen_facts = '' if len(novel_facts) == 0 else random.sample(list(novel_facts), min(len(novel_facts), facts_to_mention))
    ...
    if chosen_facts != '':
      ...
      for i, fact in enumerate(chosen_facts):
        self.mentioned_facts.append(fact) #adds the fact to a list of what has already been mentioned about that person
        self.append_inverse_fact(fact)
        fact_strings = self.fact_to_str(person, can_use_pronoun, fact)[ambiguity_level if i == 0 else 'pronoun']
        phrasings += [ (i > 0) * ', and also, ', random.choice(fact_strings)]
      #use an infix if there's more facts to come
      references = self.person_references(person, can_use_pronoun, not mentioned_first_time, infix=infix)[ambiguity_level]
      #print('phrasing: ', phrasings)
      utterance = ''.join([prefix, random.choice(references), *phrasings, postfix])
    else:
      references = self.person_references(person, can_use_pronoun, not mentioned_first_time)[ambiguity_level]
  ...
  def introduce_multiple(self, people, key_person, ambiguity_level):
    ...
    OTHER_REFS = {
        ...
        'single': ['Here is {name}. Oh, and... ', 'Here is a picture of {name}. Also... '],
        #if there are many
        'multiple': ['Here are {people}. ', 'This is a photo of {people}. ', 
  ...
  def append_inverse_fact(self, fact:Relation):
      ...
      INVERSE_RELATIONS = { #maps the name of the relationship going the other way
          ...
          'unmarried_partner': {'male': 'unmarried_partner', 'female': 'unmarried_partner'},
  ...
  def person_references(self, person, can_pronoun, mentioned_before, infix=''):
      ...
      references = {None:[]}
      references['full_name'] = [nphrase.format(name = person.data['full_name'], infix=infix)
          for nphrase in PREFIXES['name']]
      
      references['pronoun'] = [pphrase.format(
              pronoun='he' if person.data['sex_or_gender'] == 'male' else 'she',
              infix=infix) 
          for pphrase in PREFIXES['pronoun']]
      name = self.get_person_name(person)
      references['name'] = [nphrase.format(name = name, infix=infix)
          for nphrase in PREFIXES['name']]
      
      references[None] = references['full_name'] + references['name'] + references['pronoun']
      return references
  ...
  def fact_to_str(self, person, can_pronoun, fact):
      ...
      fstr = ' '.join([name, random.choice(rel_phrasings)]) #maybe add more to this depending on the relation: a wife sounds strange
      result = {None: [fstr], 
                'name': [fstr], 
                'full_name': [' '.join([person.data['full_name'], random.choice(rel_phrasings)])]}
      #print('fstr: ', fstr)
      if can_pronoun:
        result['pronoun'] = [' '.join(['he' if person.data['sex_or_gender'] == 'male' else 'she', 
  ...
  def relation_phrasings(self, person, rel):
    ...
    if other == self.speaker.data['full_name']:
      ...
      s = "'s"
    phrasings = [tphrase.format(other=other, rel_name=rel_name, s=s) for tphrase in TYPE_PHRASINGS[rel.type]]

    return phrasings  

INTRODUCTIONS = {
    'pronoun': ['Here they are! ', 'Oh, here they are. ', 'Here are they. ', 'Would you look at them. ']
}
PREFIXES = {
  'pronoun': ['Here {pronoun} is again. {infix}', 'Here {pronoun} is, '],
  'name': ["Here's {name}. ", "Here's {name}! {infix}", "Here you see {name}. {infix}",
           "Here you see {name}. {infix}",
           "This picture shows {name}. {infix}", "Look, here's {name}. {infix}"]
}
  ...
  for picture_dir in os.listdir(scenario_path):
    for file_name in os.listdir(os.path.join(scenario_path, picture_dir)):
      if file_name.endswith('.json'):
        ...
        people_in_photo = extract_persons(os.path.join(scenario_path, picture_dir, file_name), from_json=True)
        utterance = generator.generate_random_utterance(Template([]), people_in_photo, ambiguity, person, facts)
