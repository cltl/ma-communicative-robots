...
model = T5ForConditionalGeneration.from_pretrained(
    "path_to_trained_model", return_dict=True
)
...
input_ids = tokenizer.encode(
    "WebNLG: sidharth | hometown | Delhi && sidharth | play |  football </s>",
    return_tensors="pt",
)  # Batch size 1
...
model.eval()
input_ids = tokenizer.encode(
    "DART: Dwayne Johnson | nickName  | The Rock && Dwayne Johnson | actor | movie </s>",
    return_tensors="pt",
)  # Batch size 1
input_ids = input_ids.to(dev)
outputs = model.generate(input_ids)
...
model.eval()
input_ids = tokenizer.encode(
    "DART: Dwayne Johnson | actor | Red Notice && Netflix | creator | Red Notice </s>",
    return_tensors="pt",
)  # Batch size 1
input_ids = input_ids.to(dev)
outputs = model.generate(input_ids)
